...
const DEFAULT_PADDING_L = 10;
const DEFAULT_PADDING_T = 50;

export const calcGroupNodesPos = (_groups, config = {}) => {
  const ROW_CNT = config.rowCnt || 5;
  let _groupsInfo = {};
  ...
  _groups.forEach((item) => {
    item.top += DEFAULT_PADDING_T;
    item.left += DEFAULT_PADDING_L;
  });
};
